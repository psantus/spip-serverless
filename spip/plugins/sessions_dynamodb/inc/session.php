...
// ── DynamoDB session helpers (Lambda) ──────────────────────────────────────
function _ddb_session_client() {
	static $client;
	if (!$client && getenv('SPIP_SESSION_TABLE')) {
...
function _ddb_session_enabled() {
	return (bool) getenv('SPIP_SESSION_TABLE');
}

function _ddb_session_put($key, $data) {
	if ($c = _ddb_session_client()) {
		$c->putItem([
			'TableName' => getenv('SPIP_SESSION_TABLE'),
			'Item' => [
				'id' => ['S' => $key],
				'data' => ['S' => $data],
				'expires' => ['N' => (string)(time() + 86400)],
			],
		]);
	}
}

function _ddb_session_get($key) {
	if ($c = _ddb_session_client()) {
		$r = $c->getItem([
			'TableName' => getenv('SPIP_SESSION_TABLE'),
			'Key' => ['id' => ['S' => $key]],
		]);
		return $r['Item']['data']['S'] ?? null;
	}
	return null;
}

function _ddb_session_delete($key) {
	if ($c = _ddb_session_client()) {
		$c->deleteItem([
			'TableName' => getenv('SPIP_SESSION_TABLE'),
			'Key' => ['id' => ['S' => $key]],
...
function _ddb_session_list($prefix) {
	if ($c = _ddb_session_client()) {
		$r = $c->scan([
			'TableName' => getenv('SPIP_SESSION_TABLE'),
		...
		return array_map(fn($i) => $i['id']['S'], $r['Items'] ?? []);
	}
	return [];
}
// ── End DynamoDB helpers ───────────────────────────────────────────────────
...
function supprimer_sessions($id_auteur, $toutes = true, $actives = true) {
	...
	if (_ddb_session_enabled()) {
		if ($actives) {
			$keys = _ddb_session_list($id_auteur . '_');
			foreach ($keys as $key) {
				_ddb_session_delete($key);
			}
		}
		spip_session(true);
		return;
...
function lister_sessions_auteur($id_auteur, $nb_max = null) {
	...
	if (_ddb_session_enabled()) {
		$keys = _ddb_session_list($id_auteur . '_');
		// Write to tmp so callers can include() them
		$sessions = [];
		foreach ($keys as $key) {
			$tmpfile = '/tmp/spip/sessions/' . $key;
			if (!file_exists($tmpfile)) {
				$data = _ddb_session_get($key);
				if ($data) {
		...
		// Limit sessions
		if ($id_auteur && count($sessions) > $nb_max) {
			$to_delete = array_slice($sessions, 0, count($sessions) - $nb_max);
			foreach ($to_delete as $s) {
				_ddb_session_delete(basename($s));
				@unlink($s);
			}
			$sessions = array_slice($sessions, count($sessions) - $nb_max);
		}
		return $sessions;
	}
...
function ecrire_fichier_session($fichier, $auteur) {
	...
	if (_ddb_session_enabled()) {
		_ddb_session_put(basename($fichier), $texte);
		return true;
	}
...
function fichier_session($alea, $tantpis = false) {
	...
	if (empty($GLOBALS['meta'][$alea])) {
		...
		$filename = intval($c) . '_' . md5($c . ' ' . $GLOBALS['meta'][$alea]) . '.php';

		if (_ddb_session_enabled()) {
			// Return a temp file path — caller may include() it or check file_exists()
			$tmpfile = '/tmp/spip/sessions/' . $filename;
			if (!file_exists($tmpfile)) {
				$data = _ddb_session_get($filename);
				if ($data) {
					@mkdir(dirname($tmpfile), 0777, true);
					file_put_contents($tmpfile, $data);
				}
